...
namespace ServiceFines.Controllers
{
    ...
    public class GatewayController : ApiController
    {
        ...
        private async Task<int> DeleteFine(FineModel fine)
        {
            try
            {
                using (HttpClient client = new HttpClient())
        ...
        private async Task<int> DeleteUser(UserModel user)
        {
            try
            {
                using (HttpClient client = new HttpClient())
        ...
        public async Task<IHttpActionResult> Get([FromUri] UserModel user)
        {
            ...
            logger.Info("Success compliete request GET from {3} with parametrs  'FIO'= {0} 'Phone'= {1} 'Adress'= {2}", user.FIO, user.Phone, user.Adress, ip);
            return Ok(UserInfo);
        }
        ...
        public async Task<IHttpActionResult> Get([FromUri] MachineModel machine)
        {
            ...
            logger.Info("Success compliete request GET from {4} with parametrs 'Type'= {0} 'Mark'= {1} 'Model'= {2} 'Year'= {3}", machine.Type, machine.Mark, machine.Model, machine.Year, ip);
            return Ok(MachineInfo);
        }
        ...
        public async Task<IHttpActionResult> GetAll([FromUri]string UserFIO)
        {
            ...
            catch (HttpException ex)
            {
                ...
                temp.AmountFine = 0;
                logger.Info("DEGRADATE compliete request GET from {1} with parametr 'CompanyName'= {0}", UserFIO, ip);
                return Ok(temp);
            }
            ...
            logger.Info("Success compliete request GET from {1} with parametr 'UserFIO'= {0}", UserFIO, ip);
            return Ok(DUserInfo);
        }
        ...
        public async Task<IHttpActionResult> Post([FromBody]DetailUserModel value)
        {
            ...
            catch (HttpRequestException ex)
            {
                ...
                //return StatusCode(HttpStatusCode.BadGateway);
                if(flag)
                    if (DeleteFine(bufFine).Result < 0)
                    {
            ...
            catch (HttpRequestException ex)
            {
                ...
                if (DeleteUser(bufUser).Result < 0)
                {
                    return Content((HttpStatusCode)418, "Global system error. Sorry.");
                }
                return Content(HttpStatusCode.BadGateway, "Error in system. Sorry.");
